...
void sensorsRead(){
    for (int i = 0; i <= 10; i++)
    {
        leituraLumi = leituraLumi + analogRead(ANALOGINPUT);
        delay(100);
    }
    ...
    for (int i = 0; i <= 10; i++)
    {
        leituraUmiSolo = leituraUmiSolo + analogRead(ANALOGINPUT);
        delay(10);
    }
    ...
    if (leituraLumi > PARAMETRO && toggleIf) {
        t_luz = millis();
        toggleIf = false;
    }
    else if (leituraLumi < PARAMETRO) {
        t_luz = 0;
        toggleIf = false;
        32
    }
